...
class FileGest {
    private static String pulisciStringa (String S) {
        ...
        //for (int a=0;a<S.length(); a++) if (S.charAt(a) =='\u0000') return "??";
        //in realta' sarebbe meglio un non numero=> ascii compreso tra .. e tra ...
        return S;
    }
        ...
        try{
            ...
            for (int iz = 0; iz<m.Z; iz ++) { for (int ix = 0; ix< m.X; ix ++) { for (int iy = 0; iy< m.Y;iy ++) {
                if (m.point[ix][iy][iz].d != 0) {
                    System.out.println(conta+++"/"+valori.size());
    ...
    public static void writeClusters(molecola m) {
